...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // Check for fake session cookie
    if (document.cookie.includes("sirus_session=true")) {
      setIsLoggedIn(true);
    }
  ...
  const handleSignOut = () => {
    document.cookie = "sirus_session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    setIsLoggedIn(false);
    router.push("/");
    router.refresh();
  };
        ...
        <div className="hidden items-center gap-3 md:flex">
          {isLoggedIn ? (
            <button
              ...
              className="rounded-lg px-4 py-2 text-sm font-medium text-ink/70 transition-colors hover:text-danger"
            >
              Sign Out
            </button>
          ) : (
          ...
          <div className="mt-3 flex flex-col gap-2 border-t border-border-light pt-3">
            {isLoggedIn ? (
              <button
                ...
                className="rounded-lg px-4 py-2.5 text-center text-sm font-medium text-ink/70 transition-colors hover:text-danger"
              >
                Sign Out
              </button>
            ) : (
